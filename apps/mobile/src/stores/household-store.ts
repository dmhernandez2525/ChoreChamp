...
function cachedHouseholdToHousehold(cached: typeof schema.cachedHouseholds.$inferSelect): Household {
  return {
    ...
    themeId: cached.themeId || 'classic',
    whiteLabelEnabled: cached.whiteLabelEnabled ?? false,
    brandingName: cached.brandingName || null,
    brandingLogoUrl: cached.brandingLogoUrl || null,
...
function cachedMemberToMember(cached: typeof schema.cachedMembers.$inferSelect): Member {
  return {
    ...
    canRedeemRewards: cached.role !== 'caregiver',
    requiresApproval: cached.role === 'child',
    caregiverPermissions: cached.caregiverPermissions ? JSON.parse(cached.caregiverPermissions) : null,
    linkedMemberId: cached.linkedMemberId || null,
    crossHouseholdSettings: cached.crossHouseholdSettings ? JSON.parse(cached.crossHouseholdSettings) : null,
